...
function solution(N, users) {
  ...
  for (let i = 1; i <= N; i++) stageFailed[i] = 0;
  users.forEach((el) => {
    if (el <= N) stageFailed[el]++;
  });
  for (val in stageFailed) {
    stageFailed[val] = stageFailed[val] / numberUser;
    numberUser -= stageFailed[val];
  }
  answer = Object.keys(stageFailed).sort(function (a, b) {
    return stageFailed[b] - stageFailed[a];
  });
  return answer;
}
